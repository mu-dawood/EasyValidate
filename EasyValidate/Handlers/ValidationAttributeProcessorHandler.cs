        ...
        public void ProcessPropertyValidation(StringBuilder sb,ITypeSymbol type, string memberName, string chainName, List<AttributeInfo> attributes)
        {
            ...
            if (attributes.Count > 0)
            {
                ...
                for (int i = 0; i < attributes.Count; i++)
                {
                    ...
                    var outputVariable = $"{attributeName}_Output".ToSakeCase();

                    // Generate the AddValidtor call with flow control
                    if (attr.AttributeClass.IsNotNullAttribute())
                        sb.AppendLine($"{indent}if (!chain.AddValidtor({attributes[i].InstanceName}, {currentInputVariable}, out var {outputVariable}))");
                    else if (attr.AttributeClass.IsGeneralAttribute())
                        sb.AppendLine($"{indent}if (!chain.AddValidtor<{currentType.GetFullName()}>({attributes[i].InstanceName}, {currentInputVariable}, out var {outputVariable}))");
                    else
                        sb.AppendLine($"{indent}if (!chain.AddValidtor<{resolvedType!.InputType.GetFullName()}, {resolvedType!.ResolveOutPutType().GetFullName()}>({attributes[i].InstanceName}, {currentInputVariable}, out var {outputVariable}))");
