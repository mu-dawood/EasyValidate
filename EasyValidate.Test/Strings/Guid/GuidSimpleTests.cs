namespace EasyValidate.Test.Strings.Guid
{
    public class GuidSimpleTests
    {
        ...
        public void Guid_InvalidGuidString_ReturnsInvalidResult()
        {
            ...
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.UserId)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.SessionId)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalGuid)));
            var userIdErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.UserId))).ToList();
        ...
        public void Guid_InvalidGuidFormats_ReturnsInvalidResult()
        {
            ...
            Assert.True(result.HasErrors());
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.UserId)));
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.SessionId)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalGuid)));
        }
    }
}
