...
public class NumericSimpleTests
{
    ...
    public void Numeric_InvalidNonNumericString_ReturnsInvalidResult()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Amount)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.Percentage)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalValue)));
        var amountErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.Amount))).ToList();
    ...
    public void Numeric_MultipleInvalidValues_ReturnsMultipleErrors()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Amount)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Percentage)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.OptionalValue)));
    }
