...
public class AlphaSimpleTests
{
    ...
    public void Alpha_ValidString_ReturnsValidResult()
    {
        ...
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.FirstName)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.LastName)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.MiddleName)));
    }
    ...
    public void Alpha_InvalidStringWithNumbers_ReturnsInvalidResult()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.FirstName)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.LastName)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.MiddleName)));
        var firstNameErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.FirstName))).ToList();
    ...
    public void Alpha_InvalidStringWithSpecialCharacters_ReturnsInvalidResult()
    {
        ...
        Assert.True(result.HasErrors());
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.FirstName)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.LastName)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.MiddleName)));
