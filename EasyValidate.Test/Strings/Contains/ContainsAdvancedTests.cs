...
public class ContainsAdvancedTests
{
    ...
    public void NestedModel_WithInvalidMainProperty_ShouldReturnInvalid()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Any(p => p == nameof(model.MainProperty)));
    }
    ...
    [InlineData("required text", true)]
    [InlineData("ReQuIrEd text", true)]
    [InlineData("REQUIRED text", false)]
    [InlineData("ReQuIrEd text", false)]
    public void CaseInsensitiveFlag_ShouldWorkCorrectly(string input, bool expectedValid)
    {
        ...
        if (expectedValid)
        {
            Assert.True(result.IsValid() || !result.Errors.Any(e => e.Path.Any(p => p == nameof(model.CaseInsensitive))));
        }
        else
        {
            Assert.Contains(result.Errors, e => e.Path.Any(p => p == nameof(model.CaseInsensitive)));
        }
    }
    ...
    public void CustomFormatter_WithContainsErrors_ShouldFormatCorrectly()
    {
        ...
        // Act
        var result = model.Validate(formatter: formatter);

        // Assert
        Assert.False(result.IsValid());
        Assert.True(result.HasErrors());
        var formattedMessage = result.ToString();
        Assert.Contains("[TestFormatter]", formattedMessage);
    }
    ...
    public void WhitespaceSearchText_ShouldWorkCorrectly(string whitespace)
    {
        ...
        if (whitespace == " ")
        {
            Assert.True(result.IsValid() || !result.Errors.Any(e => e.Path.Any(p => p == nameof(model.WhitespaceContains))));
        }
    }
