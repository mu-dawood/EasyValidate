...
namespace EasyValidate.Test.Strings.CreditCard
{
    public class CreditCardSimpleTests
    {
        ...
        public void CreditCard_InvalidCreditCardString_ReturnsInvalidResult()
        {
            ...
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.CardNumber)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.PaymentCard)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalCard)));
            var cardErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.CardNumber))).ToList();
        ...
        public void CreditCard_InvalidCardFormats_ReturnsInvalidResult()
        {
            ...
            Assert.True(result.HasErrors());
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.CardNumber)));
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.PaymentCard)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalCard)));
        }
