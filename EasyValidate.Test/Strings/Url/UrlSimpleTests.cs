namespace EasyValidate.Test.Strings.Url
{
    public class UrlSimpleTests
    {
        ...
        public void Url_InvalidUrlString_ReturnsInvalidResult()
        {
            ...
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Homepage)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.ApiEndpoint)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalUrl)));
            var homepageErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.Homepage))).ToList();
        ...
        public void Url_InvalidUrls_ReturnsInvalidResult()
        {
            ...
            Assert.True(result.HasErrors());
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Homepage)));
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.ApiEndpoint)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalUrl)));
        }
    }
}
