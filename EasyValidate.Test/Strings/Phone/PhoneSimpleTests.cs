namespace EasyValidate.Test.Strings.Phone
{
    public class PhoneSimpleTests
    {
        ...
        public void Phone_InvalidPhoneString_ReturnsInvalidResult()
        {
            ...
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.ContactNumber)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.EmergencyContact)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalPhone)));
            var contactErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.ContactNumber))).ToList();
        ...
        public void Phone_InvalidPhoneFormats_ReturnsInvalidResult()
        {
            ...
            Assert.True(result.HasErrors());
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.ContactNumber)));
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.EmergencyContact)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalPhone)));
        }
    }
}
