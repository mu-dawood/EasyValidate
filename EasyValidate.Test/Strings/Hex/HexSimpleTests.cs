namespace EasyValidate.Test.Strings.Hex
{
    public class HexSimpleTests
    {
        ...
        public void Hex_InvalidHexString_ReturnsInvalidResult()
        {
            ...
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.ColorCode)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.HashValue)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalHex)));
            var colorCodeErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.ColorCode))).ToList();
        ...
        public void Hex_InvalidCharacters_ReturnsInvalidResult()
        {
            ...
            Assert.True(result.HasErrors());
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.ColorCode)));
            Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.HashValue)));
            Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.OptionalHex)));
        }
    }
}
