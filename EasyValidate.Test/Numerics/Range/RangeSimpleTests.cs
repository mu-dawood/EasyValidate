...
public class RangeSimpleTests
{
    ...
    public void Range_ValueBelowMinimum_ReturnsInvalidResult()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Age)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.Rating)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.Temperature)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.Score)));
        var ageErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.Age))).ToList();
        Assert.Single(ageErrors);
        Assert.Contains("must be between", ageErrors[0].FormattedMessage);
    }
    ...
    public void Range_MultipleInvalidValues_ReturnsMultipleErrors()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Age)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Rating)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Temperature)));
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Score)));
    }
}
