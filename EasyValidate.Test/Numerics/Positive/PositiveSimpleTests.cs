...
public class PositiveSimpleTests
{
    ...
    public void Positive_InvalidZeroValue_ReturnsInvalidResult()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.Contains(nameof(model.Count)));
        Assert.DoesNotContain(result.Errors, e => e.Path.Contains(nameof(model.Price)));
        ...
        var countErrors = result.Errors.Where(e => e.Path.Contains(nameof(model.Count))).ToList();
