...
public class FutureDateSimpleTests
{
    ...
    public void FutureDate_InvalidPastDate_ReturnsInvalidResult()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.EventDate) }));
        Assert.DoesNotContain(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.DeadlineDate) }));
        Assert.DoesNotContain(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.ScheduledDate) }));
        
        var eventDateErrors = result.Errors.Where(e => e.Path.SequenceEqual(new[] { nameof(model.EventDate) })).ToList();
    ...
    public void FutureDate_NullableValue_ReturnsValidResult()
    {
        ...
        // Assert
        Assert.DoesNotContain(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.DeadlineDate) }));
        Assert.True(result.IsValid() || !result.Errors.Any());
    }
    ...
    public void FutureDate_MultipleInvalidDates_ReturnsMultipleErrors()
    {
        ...
        Assert.Contains(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.EventDate) }));
        Assert.Contains(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.DeadlineDate) }));
        Assert.Contains(result.Errors, e => e.Path.SequenceEqual(new[] { nameof(model.ScheduledDate) }));
    }
